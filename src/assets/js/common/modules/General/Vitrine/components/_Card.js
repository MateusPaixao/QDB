...
class Card extends React.Component {
  ...
  setAvaliable() {
    ...
    if (
      ...
      this.state.Sku.sellers[0].commertialOffer.Price == 0 ||
      this.state.Sku.sellers[0].commertialOffer.ListPrice == 0
    ) {
      avaliable = false;
    }
  ...
  setDiscount() {
    ...
    if (this.state.Avaliable == true) {
      ...
      if (discount != 0) {
        discount = Math.round(
          (discount * 100) / this.state.Sku.sellers[0].commertialOffer.ListPrice
        );
      }
    } else {
      discount = 0;
  ...
  getImgSku(sku, size) {
    ...
    if (skuImg.length > 0) {
      // return skuImg
      return skuImg[0].imageTag
        .match(/([^">]+)"*\.(?:jpg|gif|png)/)[0]
        .allReplace({ '#width#': size, '#height#': size, '~': '' });
    } else {
      return 'noImg';
    }
  }
    ...
    const changeSku = e => {
      this.setState(
        ...
        () => {
          ...
          e.parentElement
            .querySelector('.cardProduct__config__list__item.selected')
            .classList.remove('selected');
          for (
            let i = 0;
            i < e.parentElement.querySelectorAll('.cardProduct__config__list__item').length;
            i++
          ) {
            ...
            {this.props.info.items.map(
              sku =>
                sku.itemId == this.props.skuHighlight && (
                  <span
                    className="cardProduct__config__selected"
                    style={{
                      backgroundImage: `url(${this.state.SelectedSkuThumb})`,
                      backgroundPosition: 'center',
                      backgroundSize: `5000%`
                    }}
                  >
                    <p className="cardProduct__config__selected__name">
                      {this.state.Sku['Escolha a Cor']}
                    </p>
                  </span>
                )
            )}
            ...
            <ul className="cardProduct__config__list">
              {this.props.info.items.map((sku, i) => (
                ...
                >
                  ...
                  <img
                    className={`${sku.itemId == this.props.skuHighlight ? 'selected' : ''}`}
                    data-src={this.getImgSku(sku, '40px')}
    ...
    this.setState(
      ...
      () => {
        ...
        el.parentElement.querySelector(
          '.cardProduct__sendMe__steps__title'
        ).innerHTML = `Saiba quando <b class="cardProduct__sendMe__steps__title__product">${this.state.Sku.name}</b> ficar disponível`;
        el.parentElement.querySelector('.sendMe-action').classList.add('set--send');
        el.parentElement.querySelector('.sendMe-action').innerHTML =
  ...
  unAvaliable() {
    ...
    const permitSend = e => {
      if (filter.test(e.value)) {
        e.parentElement.parentElement
          .querySelector('.sendMe-action')
          .classList.remove('set--forbidden');
        e.parentElement.parentElement.querySelector('.sendMe-action').classList.add('set--send');
      } else {
        e.parentElement.parentElement
          .querySelector('.sendMe-action')
          .classList.add('set--forbidden');
    ...
    const EmailSend = el => {
      if (el.classList.contains('set--send')) {
        ValidateEmail(
          el.parentElement.querySelector('.form-group.group-email input[name="email"]')
        );
        if (
          el.parentElement.querySelector('.group-email').classList.contains('has-error') ||
          el.parentElement.querySelector('.group-email').classList.contains('has-warning')
        ) {
          el.parentElement
            .querySelector('.form-group.group-email')
            .classList.remove('set--sending');
        } else {
          ...
          el.innerHTML = `<svg class="mailsvg" viewBox="0 0 40 17" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          new Promise(resolve => {
            ...
            let params =
              ...
              el.parentElement.querySelector('.form-group.group-email input[name="email"]').value +
              '&notifymeIdSku=' +
              this.state.Sku.itemId;
            console.log(params);
            ...
            .then(() => {
              ...
              el.parentElement.querySelector(
                '.cardProduct__sendMe__steps__title'
              ).innerHTML = `<svg class="confirmation set--fill" viewBox="0 0 82 82" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    return (
      <div className="cardProduct__sendMe">
        <span
          className="cardProduct__sendMe__close set--close"
          onClick={e => this.CloseLetMeKnow(e.currentTarget)}
        >
          x
        </span>
        <div className="cardProduct__sendMe__steps">
          <p className="cardProduct__sendMe__steps__title">
            Saiba quando{' '}
            <b className="cardProduct__sendMe__steps__title__product">{this.state.Sku.name}</b>{' '}
            ficar disponível
          </p>
        ...
        () => {
          Add().then(() => {
            ...
            setTimeout(() => {
              ...
              e.querySelector(
                '.status--adding__text'
              ).innerHTML = `${_this.state.Sku.name} foi adicionado!`;
              e.querySelector(
                '.status--adding__dots'
  ...
  render() {
    ...
    const countRating = () => {
      ...
      for (i = 1; i <= this.props.review.Rating; i++) {
        stars.push(
          <svg
            className="cardProduct__review__rating__ratingStar"
            viewBox="0 0 88 84"
            width="88"
            height="84"
            fill="none"
      ...
      while (i > this.props.review.Rating && i <= 5) {
        stars.push(
          <svg
            className="cardProduct__review__rating__ratingStar set--outline"
            viewBox="0 0 88 84"
            width="88"
            height="84"
            fill="none"
      ...
      >
            ...
            <span className={`cardProduct--change`} onClick={() => this.openConfig()}>
              ...
              <p className="cardProduct--change__close">
                <svg width="16" height="8" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15.354 4.354a.5.5 0 0 0 0-.708L12.172.464a.5.5 0 1 0-.708.708L14.293 4l-2.829 2.828a.5.5 0 1 0 .708.708l3.182-3.182zM0 4.5h15v-1H0v1z" />
                </svg>
              </p>
            </span>
            {this.mountConfig()}
        ...
        >
          <div className="cardProduct__pictureContainer">
            <img
              className="cardProduct__pictureContainer__picture"
              src={this.state.Sku.images[0].imageTag
                .match(/([^">]+)"*\.(?:jpg|gif|png)/)[0]
                .allReplace({ '#width#': '150', '#height#': '150', '~': '' })}
              alt={this.state.Sku.images[0].imageText}
              loading="lazy"
            ></img>
          ...
          <div className="cardProduct__info">
            <p className="cardProduct__info__name">{this.state.Sku.name}</p>
            ...
            <div className="cardProduct__price">
              {this.state.haveBefore != false && (
                <p className="cardProduct__price__before">
                  {'R$' +
                    this.state.Sku.sellers[0].commertialOffer.ListPrice.toFixed(2)
                      .toString()
                      .replace('.', ',')}
                </p>
              )}
              <p className="cardProduct__price__actual">
                {'R$' +
                  this.state.Sku.sellers[0].commertialOffer.Price.toFixed(2)
                    .toString()
                    .replace('.', ',')}
              </p>
              <p className="cardProduct__price__installment">
                {'até ' +
                  Math.max.apply(
                    Math,
        ...
        {this.state.Avaliable != false ? (
          <span
            className="cardProduct--addToCart status--standBy"
            onClick={e => this.addToCart(e.currentTarget)}
          >
            <svg
              className="cardProduct--addToCart__bag"
              width="20"
              height="24"
              viewBox="0 0 20 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M18.8779 6.22321C18.8046 5.43449 18.0966 4.83464 17.259 4.85168H14.7489V4.53631C14.7492 2.03134 12.6006 0.000399137 9.94984 3.99499e-05C9.94963 3.99499e-05 9.94941 3.99499e-05 9.9492 3.99499e-05C7.36904 -0.0102225 5.26862 1.95802 5.25779 4.39623C5.25758 4.44293 5.25817 4.48962 5.2595 4.53631V4.85291H2.74878C1.93116 4.85876 1.25334 5.45317 1.18619 6.22321L0.0167767 19.9383C-0.0877588 20.9886 0.299876 22.0295 1.07749 22.7865C1.81873 23.5618 2.8738 24.0029 3.97953 23.9997H16.0327C17.1408 24.0136 18.2003 23.5707 18.9347 22.7865C19.6662 22.0032 20.0466 20.9818 19.9954 19.9383L18.8779 6.22321ZM6.32086 4.53631C6.29403 2.61379 7.92153 1.03475 9.95597 1.00941C11.9904 0.984058 13.6614 2.52205 13.6882 4.44457C13.6886 4.47515 13.6886 4.50573 13.6882 4.53631V4.85291H6.32086V4.53631ZM18.1541 22.0475C17.6184 22.6231 16.8445 22.9502 16.0334 22.9441H3.97697C2.39256 22.9385 1.11301 21.7201 1.11904 20.2229C1.11936 20.1452 1.1232 20.0677 1.13056 19.9903L2.30248 6.27519C2.30573 6.03731 2.51246 5.84694 2.76419 5.85001C2.77785 5.85017 2.79145 5.85094 2.805 5.85222H5.31641V7.80374C5.31641 8.09504 5.56628 8.33118 5.87455 8.33118C6.18283 8.33118 6.4327 8.09504 6.4327 7.80374V5.85463H13.8V7.80615C13.8 8.09745 14.0499 8.33359 14.3581 8.33359C14.6664 8.33359 14.9163 8.09745 14.9163 7.80615V5.85463H17.4277C17.687 5.84853 17.9068 6.03351 17.9302 6.27755L19.1021 19.9926C19.0331 20.7573 18.6999 21.4793 18.1541 22.0475Z" />
            </svg>
            <p className={`cardProduct--addToCart__cta sku--${this.state.Sku.itemId}`}>
              Adicionar à Sacola
            </p>
          </span>
        ) : (
