...
class Card extends React.Component{
  ...
  getReviews(el, idProduct, qtd){
    new Promise((resolve, reject) => {
        let request = new XMLHttpRequest();
        let url = "https://service.yourviews.com.br/api/v2/pub/review/ReviewShelf?productids=" + this.props.productId;
        ...
        if(!!orderForm.items.length){
            orderForm.items.map((e, i) => {
                if(e.id == this.state.Sku.itemId){
                    let quantity = qtd.value + e.quantity;
                    let updateItem = {
                        index: i,
                        quantity: quantity
                    };
  ...
  render(){
    ...
    return (
      <div className={"cardProduct avaliable-" + this.state.Avaliable} data-prod={this.props.productId}>
        <span className="cardProduct--change"></span>
        <a href={"/" + this.props.linkText} className="cardProduct__link">
        ...
        <span className="cardProduct--addToCart" onClick={this.addToCart}>
