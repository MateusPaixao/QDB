...
import { getOrder, removeItem, updateItem } from '../_vtexHelpers';
...
export const bag = {
  ...
  update(items, totalizers) {
    ...
    if (el.shelf.querySelectorAll('.js--bag-old-price').length >= 1) {
      el.discount.textContent = helper.discountify(
        [...el.shelf.querySelectorAll('.js--bag-old-price')],
        [...el.shelf.querySelectorAll('.js--bag-best-price')]
      );
    ...
    target.addEventListener(
      ...
      function(event) {
        if (event.target.classList.contains('js--bag-select')) {
          ...
          updateItem(index, event.target.value).then(({ items, totalizers }) => {
  ...
  render(items, totalizers) {
    if (items.length) {
      el.shelf.innerHTML = '';
      items.forEach(item => {
        el.shelf.innerHTML += `<section class="bag-shelf__item">
                        ...
                        <div class="bag-shelf__item-info">
                            ...
                            <button class="bag-shelf__item-info-del js--bag-remove" data-index=${items.indexOf(
                              item
                            )}></button>  
                        </div>
  ...
  cupom() {
    vtexjs.checkout.getOrderForm().done(function(orderForm) {
      ...
      couponActive != null
        ? (title.innerHTML = `Cupom ativo: <span class="bag-top__coupon">${couponActive}</span>`)
        : (title.textContent = `Minha Sacola`);
    });
  }
};
