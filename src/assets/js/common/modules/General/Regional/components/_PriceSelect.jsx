class PriceModal extends React.Component {
  ...
  render() {
    return (
      <React.Fragment>
        <select className="header__region__selection" value={this.state.Region + "," + this.state.LatLng} id="selectRegiao" onChange={e => this.handleChange(e.target.value)} >
          {this.state.Regions.map(R => <option className="header__region__selection--option" value={R} key={R.split(",")[1]} > {R.split(",")[0]} </option>)}
        </select>
      </React.Fragment>
    )
  }
