...
const PaineisContainer = () => {
    ...
    useEffect(() => {
        ...
        for (let i = 1; i < bannersPaineis.textContent.split('<div class="box-banner">').length; i++) {
            ...
            let banAlt = Content.substring(Content.lastIndexOf('alt="') + 5, Content.lastIndexOf('" src='));
            // Banner['Url'] = banUrl;
            // Banner['Src'] = banImg;

            setBanners(Banners => [...Banners, <Painel Url={banUrl} Src={banImg} Alt={banAlt} key={i} />]);
        }
    }, []);
    ...
    return (
        <div className="shell paineisContainer">
            <div className="paineis">
                <div className="category-pagination">{actualIndex}/{Banners.length} </div>
