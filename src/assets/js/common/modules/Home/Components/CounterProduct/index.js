...
const Methods = {
        ...
        for (const i in skuList) {
          if (i in skuList) {
            ...
            if (sku.itemId == idSku) {
              ...
              let skuImg, skuTitle, oldPrice, newPrice, desconto, productLink;
              ...
              if (sku.sellers[0].commertialOffer.AvailableQuantity <= 0) {
                ...
                Methods.counterInit();
                document.querySelector('.w-product--wrapper--infos--parcelamento').innerHTML =
                  'até ' +
                  Math.max.apply(
                    Math,
  ...
  disableProduct: () => {
    ...
    const checkAvailable = document
      .querySelector('.w-gerador--datas')
  ...
  fetchReviews: () => {
    ...
    const url =
      'https://service.yourviews.com.br/api/v2/pub/review/ReviewShelf?productids=' + idProduto;
  ...
  AddToCart: () => {
    document
      .querySelector('.w-product--wrapper--infos--buy-button')
      .addEventListener('click', el => {
        let skuId = document.querySelector('.w-gerador--datas').getAttribute('data-sku');
        el.target.style = `pointer-events: none; opacity: .7;background-color:${document
