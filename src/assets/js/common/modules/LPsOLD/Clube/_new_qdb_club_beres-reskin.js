  ...
  ClubeOld() {
    let APIGrupoHeaders = {
      ...
      $doc.ready(function() {
        ...
        // #Close modal
        $(
          '.popup-steps.popup-form .ico-close-modal, .--close, #clubSignUp .--btn-fidelidade-confirm'
        ).on('click', function(e) {
          e.preventDefault();
          $('body').removeClass('overflow-pop-up');
      ...
      if (CPFvalida(_validaCPF) == false) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.conta) {
              $('#clubSignUp .loading-form .texto-validacao').text(
    ...
    function ValidaEmail(_email) {
      ...
      if (_email == '' || _email == null) {
        ...
        $('#clubSignUp .group-email small').text(
    ...
    $('#clubSignUp input[name="email"]').focus(function() {
      $('#clubSignUp .group-email input[name="email"]').attr(
        'placeholder',
    ...
    function validaForm(_cpf, _nome, _email, _dataNascimento, _celular) {
      ...
      if (
        document.querySelector('#clubSignUp input[name="terms"').checked ||
        $('.group-nome').hasClass('has-error') ||
        $('.group-email').hasClass('has-error') ||
        $('.group-datenasc').hasClass('has-error') ||
        $('.group-cel').hasClass('has-error')
            ...
            if (resp) {
              ...
              createFidelidade(_dataObject, '1', function(bool, data) {
                if (bool) {
                  ...
                  statusAtualiza =
                    dataAtualiza['soap:Envelope']['soap:Body'].CriaClienteSobrenomeResponse
                      .CriaClienteSobrenomeResult.Status;

                  if (statusAtualiza == 3288334773) {
                    $('#clubSignUp .loading-form .texto-validacao').text(
                      'O e-mail ' + _dataObject.email + ' já está em uso!'
                    );
                  } else if (statusAtualiza == 3254845440) {
                    $('#clubSignUp .loading-form .texto-validacao').text(
                      'Ocorreu um erro no processamento, revise os dados e tente novamente!'
                    );
    ...
    function createFidelidade(_dataObject, atualiza, callback) {
      ...
      var dataToShow = [
        _dataObject.document,
        _dataObject.firstName.split(' ')[0] + ' ' + _dataObject.lastName,
        _dataObject.birthDate.split('/')[0] +
          '/' +
          _dataObject.birthDate.split('/')[1] +
          '/' +
          _dataObject.birthDate.split('/')[2],
        _dataObject.email
      ];
      ...
      var dataNascimento =
        _dataObject.birthDate.split('/')[2].toString() +
        '/' +
        _dataObject.birthDate.split('/')[1].toString() +
        '-' +
        _dataObject.birthDate.split('/')[0].toString();
      ...
      var telFidelidade =
        _dataObject.phone.split(')')[0].replace('(', '') +
        _dataObject.phone
          .split(')')[1]
          .replace('(', '')
          .replace('-', '');
      ...
      var data1 = {
        consumidor: {
          ...
          dataNascimento:
            dataNascimentoComplete.year +
            '-' +
            dataNascimentoComplete.month +
            '-' +
            dataNascimentoComplete.day,
      ...
      var perguntas = [
        {
          ...
          mensagem:
            'a cada R$1 gasto em uma das nossas lojas, você ganha 1 ponto na sua conta do clube das berês fidelidade.'
        },
        {
          title: 'como eu troco meus pontos?',
          mensagem:
            'a cada 20 pontos acumulados, você tem direito a R$ 1,00 de crédito no pagamento de qualquer produto ou serviÃ§o. aí funciona assim: você pode usar os pontos pra ter um desconto na sua compra ou pagar ela inteira! mas olha, você só pode fazer essa troca quando tiver o saldo mínimo de 100 pontos, ok?'
        },
        {
          title: 'como funcionam as promoçãµes exclusivas?',
          mensagem:
            'a gente sempre prepara promoçãµes exclusivas pras consumidoras cadastradas no clube das berês <3 pra saber delas é só ficar de olho nos e-mails que a gente envia, nas publicaçãµes na nossa página no Facebook e no nosso site (ah! pra ter acesso Ã s promos na nossa loja virtual, você precisa fazer o login na sua conta, tá?). <br/>se você for comprar em uma das nossas lojas físicas, você precisa levar um print do nosso e-mail ou da nossa publicação no Facebook pra mostrar pra vendedora.'
        },
        {
          title: 'tem presente de aniversário?',
          mensagem:
            'no mês de aniversário das berês, a gente sempre manda um e-mail com uma surpresa! é só ficar de olho nas nossas mensagens ;) não esquece que pra participar você precisa levar um print do e-mail na loja, combinado?'
        },
        {
          title: 'como eu me cadastro?',
          mensagem:
            "pra fazer seu cadastro é super fácil: é só dar uma passadinha em uma das nossas lojas e conversar com uma das vendedoras, ou se cadastrar <a href='' class='pop-cadastro'>aqui no nosso site</a>. ah! você não precisa comprar nada pra se cadastrar!"
        },
        {
          title: 'até quando valem meus pontos?',
          mensagem:
            'os pontos acumulados no clube das berês fidelidade tem o prazo de validade de 1 ano.'
        }
      ];
        ...
        .on('click', function(event) {
          ...
          if (isAppended) {
            ...
            perguntas.forEach(function(item, idx) {
              ...
              htmlTotal += htmlItem
                .replace('#pergunta#', item.title)
                .replace('#resposta#', item.mensagem);
              //}
            });
