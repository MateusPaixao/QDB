...
const Pictures = ({ Sku }) => {
  ...
  const [MainPicStyles, setMainPicStyles] = React.useState({
    backgroundImage: `url(${mainPicture.allReplace({
      '#width#': '600',
      '#height#': '600',
      '~': ''
    })})`,
  ...
  const setMain = (e, src) => {
    ...
    setMainPicStyles({
      ...MainPicStyles,
      backgroundImage: `url(${src.allReplace({
        '#width#': '600',
        '#height#': '600',
        '~': ''
      })})`
  ...
  //   setMainPicStyles({ ...MainPicStyles, backgroundPosition: `${x}% ${y}%` });
  // };
