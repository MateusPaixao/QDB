...
function validReceiver(){
    const urlShipping = window.location.href.indexOf('checkout/#/shipping') > -1;
    if(urlShipping){
        const inputText = document.querySelector("#ship-receiverName");
        const goToPayment = document.querySelector('#btn-go-to-payment')
        inputText.addEventListener('keyup', function({currentTarget}) {

            let str = currentTarget.value;
            let lastStr = str.substr(str.length - 1);
            let isNumber = /\d/.test(lastStr);
            let newString = [...str];
            const regex = '[-=@!#$%¨&*+_´`^~;:?áÁéÉíÍóÓúÚãÃçÇ|\?,./{}"<>()]';

            let checkInputValue = inputText.value.match(regex)
