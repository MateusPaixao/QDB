            ...
            .then((product) => {
                ...
                for (const i in skuList) {
                    if (skuList.hasOwnProperty(i)) {
                        const sku = skuList[i];
                        if(sku.itemId == idSku){
                            let skuImg, skuTitle, oldPrice, newPrice, parcelamento;
                            skuImg = document.querySelector('.w-product--wrapper--img');
                            skuImg.src = sku.images[0].imageUrl;
                            skuTitle = document.querySelector('.w-product--wrapper--title');
                            skuTitle.textContent = sku.name;
                            oldPrice = document.querySelector('.w-product--old-price');
                            oldPrice.textContent = `R$ ${sku.sellers[0].commertialOffer.PriceWithoutDiscount.toFixed(2).replace('.',',')}`;
                            newPrice = document.querySelector('.w-product--wrapper--new-price');
                            newPrice.textContent = `R$ ${sku.sellers[0].commertialOffer.Price.toFixed(2).replace('.',',')}`;
                        }
                    }
                }
            })
